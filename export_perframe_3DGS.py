...
parser.add_argument("--export_camera_set", default="video", choices=["video", "train", "test"], help="Camera split used to sample timestamps for per-frame Gaussian export.")
parser.add_argument("--max_frames", default=0, type=int, help="Export only the first N unique timestamps. 0 means all.")
...
if args.export_camera_set == "train":
    export_cameras = scene.getTrainCameras()
elif args.export_camera_set == "test":
    export_cameras = scene.getTestCameras()
else:
    export_cameras = scene.getVideoCameras()

unique_viewpoints = []
seen_times = set()
for viewpoint in export_cameras:
    time_key = round(float(viewpoint.time), 8)
    if time_key in seen_times:
        continue
    seen_times.add(time_key)
    unique_viewpoints.append(viewpoint)

unique_viewpoints = sorted(unique_viewpoints, key=lambda cam: float(cam.time))
if args.max_frames > 0:
    unique_viewpoints = unique_viewpoints[: args.max_frames]
print(f"Exporting {len(unique_viewpoints)} unique timestamps from {args.export_camera_set} cameras.")

for index, viewpoint in enumerate(unique_viewpoints):
