...
class GaussianModel:
    ...
    def birth_from_existing(self, selected_indices, offsets, opacity_scale=0.5, scale_shrink=0.8):
        ...
        offsets = offsets.to(self._xyz.device, dtype=self._xyz.dtype)

        new_xyz = self._xyz[selected_indices].detach() + offsets
        new_features_dc = self._features_dc[selected_indices].detach()
        new_features_rest = self._features_rest[selected_indices].detach()
        new_opacities = self._opacity[selected_indices].detach()
