...
OUTPUT=${OUTPUT:-${MCC_SCRIPTS}/4dgs_output/unity_test_02_r3_10frames_mcc_motion_${VERSION}_iter${ITERATIONS}}
CONFIG=${CONFIG:-arguments/movable_camera_cluster.py}
PORT=${PORT:-6033}

if [ ! -d "${DATASET}/sparse/0" ]; then
  echo "[${VERSION}] Creating 10-frame dataset: ${DATASET}"
...
"${PYTHON_BIN}" train.py \
  ...
  --mcc_ray_opacity_scale 0.30 \
  --mcc_ray_scale_shrink 0.75 \
  --mcc_hypothesis_gate \
  --mcc_hypothesis_initial_prob 0.35 \
  --mcc_hypothesis_lr 2.0 \
  --mcc_hypothesis_mature_prob 0.70 \
  --mcc_hypothesis_prune_prob 0.08 \
  --mcc_hypothesis_max_groups_per_verify 4 \
  --mcc_hypothesis_local_evidence \
  --mcc_hypothesis_support_mode projection \
  --mcc_hypothesis_projection_radius 28 \
  --mcc_hypothesis_projection_max_points 128 \
  --mcc_hypothesis_projection_max_area 0.20 \
  --mcc_hypothesis_support_quantile 0.90 \
  --mcc_hypothesis_support_min 128 \
  --mcc_hypothesis_residual_weight 0.50 \
  --mcc_hypothesis_edge_weight 0.25 \
  --mcc_hypothesis_normalize \
