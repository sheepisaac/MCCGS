...
"${PYTHON_BIN}" train.py \
  ...
  --mcc_motion_sample_points 3072 \
  --mcc_motion_max_propagated_points 192 \
  --mcc_motion_confidence_threshold 0.18 \
  --mcc_verify_hypotheses \
  --mcc_verify_interval 500 \
  --mcc_verify_min_tests 3 \
  --mcc_verify_views 3 \
  --mcc_verify_adaptive \
  --mcc_verify_adaptive_min_abs 0.000002 \
  --mcc_verify_adaptive_mad_scale 1.5 \
  --mcc_verify_sign_ratio 0.65 \
  --mcc_verify_history 16 \
